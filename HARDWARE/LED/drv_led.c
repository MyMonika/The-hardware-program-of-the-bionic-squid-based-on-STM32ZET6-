...
void drv_led_init( void )
{
	...
	//使能口线时钟
	RCC_APB2PeriphClockCmd( LED_RED_GPIO_CLK | LED_BLUE_GPIO_CLK, ENABLE );	//打开端口时钟，是GPIOB与GPIOE
	
	GpioInitStructer.GPIO_Mode = GPIO_Mode_Out_PP;		//希辰注：配置推挽输出				
	GpioInitStructer.GPIO_Speed = GPIO_Speed_2MHz;		
	
	GpioInitStructer.GPIO_Pin = LED_RED_GPIO_PIN;		//希辰注：这里是5引脚
	...
	GpioInitStructer.GPIO_Pin = LED_BLUE_GPIO_PIN;		//希辰注：这里是5引脚
